...
docker build -t ds4u/multi-worker:latest -t ds4u/multi-worker:$SHA -f ./worker/Dockerfile ./worker
docker push ds4u/multi-client:latest
docker push ds4u/multi-server:latest
docker push ds4u/multi-worker:latest
docker push ds4u/multi-client:$SHA
docker push ds4u/multi-server:$SHA
docker push ds4u/multi-worker:$SHA
kubectl apply -f k8s
